...
from .swin_transformer4_cross_attn_ResB_v2_student_colab import SwinTransformer4 as swin_cross_attn_RESB_v2_student_colab  ## Proposed
...
def build_model(config):
    ...
    if model_type == 'swin':
        ...
        model = swin_cross_attn_RESB_v2_student_1(pre_step=config['pre_step'],
                                ...
                                patch_norm=config['SWIN.PATCH_NORM'],
                                use_checkpoint=config['TRAIN.USE_CHECKPOINT'])
    elif model_type == 'swin_cross_attn_ResB_v2_student_colab':
        model = swin_cross_attn_RESB_v2_student_colab(pre_step=config['pre_step'],
                                img_size=config['IMG_SIZE'],
                                patch_size=config['SWIN.PATCH_SIZE'],
                                in_chans=config['SWIN.IN_CHANS'],
                                embed_dim=config['SWIN.EMBED_DIM'],
